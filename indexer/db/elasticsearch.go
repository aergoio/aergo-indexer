...
func (esdb *ElasticsearchDbController) InsertBulk(documentChannel chan doc.DocType, params UpdateParams) (uint64, error) {
	...
	for d := range documentChannel {
		...
		if bulk.NumberOfActions() >= params.Size {
			...
			if res.Errors {
				return total, errors.New("bulk commit failed")
			}
		}
...
func (esdb *ElasticsearchDbController) SelectOne(params QueryParams, unmarshal UnmarshalFunc) (doc.DocType, error) {
	ctx := context.Background()
	...
	// Unmarshall document
	hit := res.Hits.Hits[0]
	unmarshalled, err := unmarshal(*hit.Source)
...
func (esdb *ElasticsearchDbController) CreateIndex(indexName string, documentType string) error {
	ctx := context.Background()
	createIndex, err := esdb.Client.CreateIndex(indexName).BodyString(doc.Mappings[documentType]).Do(ctx)
	if err != nil {
		return err
	}
...
func (esdb *ElasticsearchDbController) Scroll(params QueryParams, unmarshal UnmarshalFunc) ScrollInstance {
...
func (scroll *EsScrollInstance) Next() (doc.DocType, error) {
	...
	if scroll.current < scroll.currentLength {
		...
		scroll.current++
		unmarshalled, err := scroll.unmarshal(*doc.Source)
		if err != nil {
			return nil, err
		}
		return unmarshalled, nil
